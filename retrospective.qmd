...
Most team members contributed meaningfully across milestones. Some variation in task counts reflects differences in task complexity (e.g., larger coding tasks vs smaller documentation or bug-fix tasks). The "No Assignees" category comes from leftover Dependabot PRs.
...
When reviewing the project board and issue timelines, we identified several process-related bottlenecks. In some cases, we did not consistently update task statuses to “In Progress,” which reduced visibility into active work and made it harder to identify capacity constraints early.
...
However, in Milestone 3 (@fig-team-cont), sjbalagit had the highest number of tasks. If they had been unavailable (e.g., sick), we would have faced delays. However in such a scenario, we could hold team meetings to redistribute tasks so that others could pick up the work.
