...
def get_antipode(location, resolve_names=True):
    ...
    if isinstance(location, str):
        ...
        if len(location) != 2:
            raise TypeError(f"Coordinate tuple must have exactly 2 elements, got {len(location)}")
        lat, lon = location
        if not isinstance(lat, (int, float)) or not isinstance(lon, (int, float)):
            raise TypeError("Coordinate values must be numeric (int or float)")
    else:
        raise TypeError(f"location must be a string or tuple of floats, got {type(location).__name__}")
    ...
    try:
        reverse_result = geolocator.reverse(f"{antipode_lat}, {antipode_lon}", language="en")
